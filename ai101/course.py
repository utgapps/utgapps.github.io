...
# ---------------------------------------------------------------- week 2 ----
{
 ...
 "ops": [
  ...
  ADD(JS, "submit", [
    ...
    "  addLine('you', text);",
    "  addLine('bot', 'You said: ' + text);   // a pretend answer, for now",
    "});",
  ]),
 ],
 "flow": [
 ...
 "slides": [
  ...
  {"title": "The browser owns the loop", "sub": "addEventListener('submit', ...)", "bullets": ["You leave a note about what to do", "The browser calls you when it happens", "Same idea, opposite direction"],
   "code": [(JS, "submit")]},
  {"title": "Checkpoint: it echoes you", "checkpoint": True, "say": "Press Run, type a message and press Send. It should show as 'you: ...' and the bot echoes it back right below. This runs entirely in the browser - no AI yet."},
...
# --- Quiz backfill, weeks 1-11 ----------------------------------------------
# One check per week, on that week's culminating block, so it lands right after
# the new topic is typed and run. A few questions on the new code plus one from
# an earlier week (spaced repetition). Authored with the correct option FIRST;
# build.balance_quiz reseats it so answers are not all "A".
QUIZZES.update({
    (1, "style.css", "composer"): [
        {"q": "In <button>Send</button>, what is 'Send'?",
         "options": ["The content shown on the button", "The tag's name",
                     "An attribute", "A CSS class"], "answer": 0,
         "why": "Whatever sits between the opening and closing tags is the content."},
        {"q": "In  body { background: #0d1b21; }  what is 'background'?",
         "options": ["The property being set", "The selector",
                     "The value", "A comment"], "answer": 0,
         "why": "A rule is  selector { property: value; }  - background is the property."},
        {"q": "Which selector targets elements with class=\"app\"?",
         "options": [".app", "app", "#app", "*app"], "answer": 0,
         "why": "A class selector starts with a dot."},
        {"q": "What does  display: flex;  do to a box's children?",
         "options": ["Lines them up in a row", "Hides them",
                     "Makes them bold", "Deletes them"], "answer": 0,
         "why": "flex lays a box's children out in a row (or a column)."},
    ],
    (2, "script.js", "submit"): [
        {"q": "What is a function?",
         "options": ["A reusable block of code you run by name", "A kind of colour",
                     "A CSS rule", "An HTML tag"], "answer": 0,
         "why": "A function is a named block of code you can run whenever you call it."},
        {"q": "form.addEventListener('submit', ...) runs your code when?",
         "options": ["The form is submitted", "The page first loads",
                     "A colour changes", "Never"], "answer": 0,
         "why": "addEventListener runs your code every time that event happens."},
        {"q": "Why call  event.preventDefault()  in a submit handler?",
         "options": ["To stop the page reloading", "To send the message twice",
                     "To clear the CSS", "To make text bold"], "answer": 0,
         "why": "Submitting a form reloads the page by default; preventDefault stops that."},
        {"q": "Which one creates a paragraph of text? (from week 1)",
         "options": ["<p>...</p>", "<button>", "body { }", ".app"], "answer": 0,
         "why": "<p> wraps a paragraph; the others are a button, a rule, and a selector."},
    ],
    (3, "script.js", "models"): [
        {"q": "What does  fetch(url)  do?",
         "options": ["Asks a server for something over the network", "Deletes a file",
                     "Creates an HTML element", "Loops over a list"], "answer": 0,
         "why": "fetch sends a request across the network and comes back with a reply."},
        {"q": "What does  await  do?",
         "options": ["Waits for the reply before the next line runs", "Skips the request",
                     "Repeats the request", "Speeds the code up"], "answer": 0,
         "why": "await pauses on that line until the slow thing finishes."},
        {"q": "{ role: 'user', content: text }  is a(n)...?",
         "options": ["object with named fields", "array", "string", "function"], "answer": 0,
         "why": "Curly braces with key: value pairs make an object."},
        {"q": "Why must a function be marked  async  here? (uses week 3 idea)",
         "options": ["So it can use await inside", "So it runs twice",
                     "So it becomes a CSS rule", "So it prints to the console"], "answer": 0,
         "why": "await only works inside a function marked async."},
    ],
    (4, "script.js", "ask"): [
        {"q": "JSON.stringify(obj)  turns an object into...?",
         "options": ["text you can send", "a number", "an HTML element", "a function"], "answer": 0,
         "why": "It packs an object into text so it can travel over the network."},
        {"q": "[ ]  in JavaScript is...?",
         "options": ["a list of items in order", "an object", "a string", "a comment"], "answer": 0,
         "why": "Square brackets make an array - a list."},
        {"q": "For  msgs[0]  which item do you get?",
         "options": ["the first one", "the last one", "a random one", "all of them"], "answer": 0,
         "why": "Indexing starts at 0, so [0] is the first item."},
        {"q": "What does  fetch  return that you must await? (from week 3)",
         "options": ["a reply that takes time to arrive", "a colour",
                     "the first item of a list", "an HTML tag"], "answer": 0,
         "why": "fetch talks to a server, which takes time - so you await the reply."},
    ],
    (5, "script.js", "explain"): [
        {"q": "In  try { ... } catch (e) { ... }  when does catch run?",
         "options": ["When the try code hits an error", "When the try code works",
                     "Always", "Never"], "answer": 0,
         "why": "catch only runs if something in try throws an error."},
        {"q": "What does  !ok  mean?",
         "options": ["NOT ok - the opposite", "ok twice", "ok and true", "ok or nothing"], "answer": 0,
         "why": "! flips a true/false value to its opposite."},
        {"q": "cond ? x : y  gives you...?",
         "options": ["x if cond is true, otherwise y", "always x", "always y",
                     "the word 'true'"], "answer": 0,
         "why": "The ternary picks the first value when the condition is true, else the second."},
        {"q": "msgs[0]  gives which item? (from week 4)",
         "options": ["the first", "the last", "a copy of all", "none"], "answer": 0,
         "why": "Counting starts at zero, so [0] is the first item."},
    ],
    (6, "style.css", "setup"): [
        {"q": "What is  <textarea>  for?",
         "options": ["Typing several lines of text", "A single click button",
                     "One short line only", "Showing an image"], "answer": 0,
         "why": "A textarea is a big multi-line typing box."},
        {"q": "<details> with <summary> makes...?",
         "options": ["a section you can open and close", "a dropdown menu",
                     "a password box", "a table"], "answer": 0,
         "why": "details/summary is a collapsible section; summary is its visible label."},
        {"q": "font-weight: 700;  makes text...?",
         "options": ["bold", "italic", "bigger", "a new colour"], "answer": 0,
         "why": "Higher font-weight means bolder text."},
        {"q": "What does try/catch let your app do? (from week 5)",
         "options": ["Keep running instead of crashing on an error", "Run faster",
                     "Skip fetch", "Reload the page"], "answer": 0,
         "why": "catch handles the error so the whole app doesn't fall over."},
    ],
    (7, "script.js", "submit"): [
        {"q": "history.push(msg)  does what to the list?",
         "options": ["Adds msg to the end", "Removes the first item",
                     "Empties the list", "Sorts the list"], "answer": 0,
         "why": ".push adds one item onto the end of an array."},
        {"q": "Why keep a  history  array?",
         "options": ["So the AI remembers past messages", "To style the page",
                     "To speed up fetch", "To hide old messages"], "answer": 0,
         "why": "Sending the whole history is how the AI 'remembers' the conversation."},
        {"q": "[...history, msg]  makes...?",
         "options": ["a new list of all old items plus msg", "an empty list",
                     "just msg", "only the first item"], "answer": 0,
         "why": "...spread tips every item of history into a new array, then adds msg."},
        {"q": "<textarea> lets you type...? (from week 6)",
         "options": ["multiple lines", "one line only", "numbers only", "nothing"], "answer": 0,
         "why": "A textarea is the multi-line typing box."},
    ],
    (8, "script.js", "trim"): [
        {"q": "arr.length  tells you...?",
         "options": ["how many items are in the list", "the first item",
                     "the last item", "the total colour"], "answer": 0,
         "why": ".length is the count of items in the array."},
        {"q": "for (const m of history)  visits...?",
         "options": ["each item in history, one by one", "only the first item",
                     "just the length", "nothing"], "answer": 0,
         "why": "for...of walks through every item in the list in turn."},
        {"q": "history.shift()  removes which item?",
         "options": ["the first one", "the last one", "a random one", "all of them"], "answer": 0,
         "why": ".shift drops the item at the FRONT of the list."},
        {"q": "history.push(x)  adds x...? (from week 7)",
         "options": ["to the end of the list", "to the start", "nowhere", "as a CSS class"], "answer": 0,
         "why": ".push always adds onto the end - the opposite end from .shift."},
    ],
    (9, "index.html", "controls"): [
        {"q": "<select> with <option>s makes...?",
         "options": ["a dropdown menu", "a text box", "a button", "a paragraph"], "answer": 0,
         "why": "A select is a dropdown; each option is one choice in it."},
        {"q": "What is  <label>  for?",
         "options": ["Naming the input beside it", "Submitting the form",
                     "Styling text", "Holding an image"], "answer": 0,
         "why": "A label names the control it sits next to."},
        {"q": "<output> is a good place to...?",
         "options": ["show a value the script sets", "type a message",
                     "load CSS", "group buttons"], "answer": 0,
         "why": "output displays a value your JavaScript fills in."},
        {"q": "for (const m of list)  runs the body...? (from week 8)",
         "options": ["once per item", "once total", "never", "forever"], "answer": 0,
         "why": "for...of runs the body once for each item in the list."},
    ],
    (10, "script.js", "stream"): [
        {"q": "JSON.parse(text)  turns text into...?",
         "options": ["a real object", "plain text", "a number only", "an HTML tag"], "answer": 0,
         "why": "JSON.parse is the reverse of stringify - text back into an object."},
        {"q": "line.split(',')  returns...?",
         "options": ["a list of the pieces", "one big string", "the first letter", "a number"], "answer": 0,
         "why": ".split cuts a string into an array at each comma."},
        {"q": "line.startsWith('data:')  checks if the line...?",
         "options": ["begins with 'data:'", "ends with 'data:'",
                     "contains a number", "is empty"], "answer": 0,
         "why": ".startsWith is true only when the string begins with that text."},
        {"q": "<select> builds a...? (from week 9)",
         "options": ["dropdown menu", "text area", "button", "label"], "answer": 0,
         "why": "select + option make a dropdown menu."},
    ],
    (11, "script.js", "submit"): [
        {"q": "el.classList.add('typing')  does what?",
         "options": ["Adds the 'typing' class to the element", "Removes every class",
                     "Sets the text", "Deletes the element"], "answer": 0,
         "why": "classList.add turns one CSS class on, leaving the others alone."},
        {"q": "A CSS  animation  on the dots makes them...?",
         "options": ["move on their own over time", "vanish forever",
                     "turn into text", "freeze the page"], "answer": 0,
         "why": "animation replays keyframes over time, so the dots keep moving."},
        {"q": "visibility: hidden  hides an element but...?",
         "options": ["keeps its space on the page", "removes its space",
                     "deletes it for good", "makes it bold"], "answer": 0,
         "why": "Unlike display:none, visibility:hidden leaves the gap where it was."},
        {"q": "JSON.parse(text)  gives you...? (from week 10)",
         "options": ["an object", "a string", "a number", "an HTML element"], "answer": 0,
         "why": "JSON.parse turns text into a usable object."},
    ],
})
