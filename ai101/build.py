...
// a signed-in teacher (the deck is same-origin with the classroom, so it reads
// the saved account token). A student has a class code, not a token, so the
// request 401s for them and the checkpoint just shows the interface. Fetched
// once and cached.
var _cpKey;
function cpDemoKey() {
  ...
  _cpKey = new Promise(function (resolve) {
    ...
    fetch(api + '/demo-key', { cache: 'no-store', headers: { authorization: 'Bearer ' + token } })
