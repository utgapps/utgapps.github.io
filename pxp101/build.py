...
Emits, all derived from the one WEEKS structure in course.py:
    ...
    week-NN.html        the code every child should have by the end of week N
    teacher.html        full teaching curriculum, minute by minute
    textbook.html       the child's book: every step, every line, chapter by chapter
    workbook.html       printable homework book (challenges and recap)
    slides/week-NN.pptx one deck per week (needs `pip install python-pptx`)
    slides/week-NN.html the same deck as a web page, with a PLAYABLE checkpoint
...
neither can the teacher guide, the textbook, the homework or the slides.

A PixelPad game is not files, it is per-object code panels, so what AI101 calls
a "file" is a panel here ("Monster loop", "Play start"). Line numbers restart
per panel, which is why a step can say "line 3 of Monster's loop" to an
eight-year-old instead of "line 147 of script.js".
"""
...
# A "file" is a PixelPad code panel - see the module docstring.
FILES = course.PANELS
...
def state_upto(week_n, done):
    """Panel contents PART-WAY through a week: every earlier week's code, plus
    only the blocks this week has already put on the board.

    An eight-year-old presses Play every few minutes, so a PXP101 hour has
    several checkpoints rather than one at the bell, and each has to run the
    game as it stands at that moment. A checkpoint that quietly ran the finished
    week would show behaviour the class has not typed yet - the slide that says
    "push it off the side, it escapes!" would not escape.
    """
    blocks = {name: list(pairs) for name, pairs in blocks_at(week_n - 1).items()}
    for kind, filename, block_id, lines in course.WEEKS[week_n - 1]["ops"]:
...
def block_spans(upto):
    ...
    This is what lets the teacher guide say "type into Monster loop, lines 3-6"
...
def chunk_block(lines, filename=""):
    """Split a panel's block into pieces of at most six lines each.

    Python is structured by indentation, so the split is too: a line ending in
    a colon travels with the indented body it runs, and a step can never show
    an `if` without the lines it controls. One indivisible unit may exceed six
    - the same allowance RULES.md makes for a single long `if` block.

    Six is the cap because past that an eight-year-old is copying rather than
    following, and because six lines is about as much as fits between two
    presses of Play.
    """
    chunks = pixelpad.paginate(list(lines), MAX_STEP_LINES)
    assert [line for chunk in chunks for line in chunk] == list(lines),         "chunking lost or reordered lines"
    return chunks
...
def code_table(filename, start, lines, marks, ident=None, tag="", gone=None,
    ...
    `range_label` replaces the computed line range, for code whose position is not fixed
    - a bonus lands after whichever earlier bonuses the child happened to take,
    so printing "lines 1-2" beside it would be a lie.
    """
    gone = gone or {}
    rows = []
    ...
    if range_label is None:
        range_label = f"line {start}" if start == end else f"lines {start}&ndash;{end}"
    ...
    return (f'<div class="snip"><div class="snip-head">'
            f'<span class="file">{esc(filename)}</span><span class="rng">{range_label}</span>'
...
def placement(week_n, filename, block_id, kind):
    ...
    ordered = [bid for bid, _lines in blocks_at(week_n)[filename]]
...
def line_sig(text):
    ...
    A comment here is set off by two-plus spaces or starts the line.
    Deliberately simple, matching the light comment style the decks use - the
    teaching is on the slides. A whole-line comment returns '' - it has no code
    to match on."""
...
def removed_in_blocks(upto):
    ...
    for name in FILES:
        ...
        for block_id, new_lines in after[name]:
            ...
            new_sigs = {sig for sig in (line_sig(line) for line in new_lines) if sig}
...
# the bonus track
#
# A bonus is prose plus, optionally, real code: week["bonus"]["lines"] is
# [(panel, [line, ...])], authored exactly as the child types it. Those lines go
# through the same kid rules as the base game and are counted against
# BONUS_BUDGET, but they are NOT ops, so they never reach state_at(), the
# milestones, the checkpoints or the slide states. That separation is the whole
# design: the child who does every bonus and the child who does none open week 9
# to the same game. See PLAN.md section 3.
# --------------------------------------------------------------------------

def bonus_lines(week):
    """[(panel, [line, ...])] for a week's bonus, or [] if it is prose only."""
    return list((week.get("bonus") or {}).get("lines") or [])


def bonus_count(week):
    return sum(len(lines) for _panel, lines in bonus_lines(week))


def bonus_code(week, ident):
    """The bonus's code, one snippet per panel, every line marked as new.

    Numbered from 1 rather than from a cumulative position, because a bonus is
    extra code on the end of a panel and where it lands depends on which earlier
    bonuses the child took. The heading says the panel, which is what they need.
    """
    snippets = []
    for index, (panel, lines) in enumerate(bonus_lines(week)):
        every_line_is_new = set(range(1, len(lines) + 1))
        snippets.append(code_table(panel, 1, list(lines), every_line_is_new,
                                   ident="%s-bonus%d" % (ident, index), tag="bonus",
                                   range_label="on the end"))
    return "".join(snippets)


def bonus_block(week, ident, heading):
    """The whole bonus card - prose, then its code if it has any."""
    if not week.get("bonus"):
        return ""
    # The body is authored as HTML - it names panels in <strong> and code in
    # <code> - so it goes out raw and unwrapped. Wrapping it in a <p> nested a
    # paragraph inside a paragraph, and escaping it printed the tags at the
    # child, which is what week-01.html did until this renderer was shared.
    return (
        '<div class="bonus"><strong>%s %s</strong>%s%s'
        '<p class="muted" style="margin:6px 0 0;font-size:13px">Nothing in a later week '
        'depends on this, so it is safe to skip and safe to keep.</p></div>'
        % (heading, esc(week["bonus"]["title"]), week["bonus"]["body"],
           bonus_code(week, ident))
    )
...
<p class="eyebrow">{TOTAL_WEEKS}-week course</p>
...
<div class="card">
  ...
  <p><a class="pill" href="textbook.html">The book</a>
     <a class="pill" href="teacher.html">Teacher curriculum</a>
     <a class="pill" href="workbook.html">Homework book</a>
     <a class="pill ghost" href="https://pixelpad.io" target="_blank" rel="noopener">Open PixelPad</a></p>
  <p class="muted" style="margin:10px 0 0;font-size:13px">The book is the one to hand a child: every
  step, every line, chapter by chapter. Each week page carries that week&rsquo;s slides and links
  straight to its lesson plan.</p>
</div>
...
def build_weeks():
    for index, week in enumerate(course.WEEKS):
        week_number = week["n"]
        prev = f'<a href="week-{week_number-1:02d}.html">&larr; Week {week_number-1}</a>' if week_number > 1 else "<span></span>"
        nxt = f'<a href="week-{week_number+1:02d}.html">Week {week_number+1} &rarr;</a>' if week_number < len(course.WEEKS) else "<span></span>"
        concepts = "".join(f'<span class="pill">{esc(concept)}</span>' for concept in week["new_concepts"])
        total = line_count(state_at(week_number))
        dropped = sum(len(removed) for name in FILES for removed in removed_lines(week_number)[name].values())
        legend = (f"The green lines are what is new since week {week_number-1}."
                  if week_number > 1 else "Everything here is new - this is week 1.")
        if dropped:
        ...
        bonus = bonus_block(week, f"w{week_number}", "Finished early?")
        body = f"""<div class="wrap">
<p class="eyebrow">Week {week_number} of {len(course.WEEKS)}</p>
<h1>{esc(week["title"])}</h1>
<p class="lead">{esc(week["big_idea"])}</p>
<p>{concepts}</p>
<p class="teach-row"><a class="pill" href="slides/week-{week_number:02d}.html">Slides for week {week_number}</a>
   <a class="pill ghost" href="slides/week-{week_number:02d}.pptx">.pptx</a>
   <a class="pill ghost" href="textbook.html#chapter-{week_number}">Chapter {week_number} of the book</a>
   <a class="pill ghost" href="teacher.html#week-{week_number}">Lesson plan for week {week_number}</a></p>
<div class="note"><strong>Where you should be by the end of this week.</strong>
{legend}
Your project is now {total} lines.</div>
{bonus}
<h2>Your code after week {week_number}</h2>
{files_view(week_number, f"w{week_number}")}
<div class="nav">{prev}{nxt}</div>
</div>"""
        write(f"week-{week_number:02d}.html", page(f"Week {week_number} · {week['title']}", body))
...
def build_teacher():
    ...
    for week in course.WEEKS:
        ...
        typed = "".join(
            '<tr><td><b>{filename}</b></td><td>{lines}</td>'
            '<td>{action}</td></tr>'.format(
                filename=esc(filename),
                lines=("line {0}".format(spans[filename][block_id][0])
                   if spans[filename][block_id][0] == spans[filename][block_id][1]
                   else "lines {0}&ndash;{1}".format(*spans[filename][block_id])),
                action="type it in fresh" if kind == "add" else "replace what is already there")
            for kind, filename, block_id, _lines in week["ops"]
        )
        typed_table = (
        ...
        bonus = bonus_block(week, f"t{week['n']}", "Bonus for fast finishers:")
...
<ul class="tight">{"".join(f"<li>{esc(objective)}</li>" for objective in week["objectives"])}</ul></div>
{typed_table}
...
<ul class="tight">{"".join(f"<li>{esc(homework['task'])}</li>" for homework in week["homework"])}</ul>
</section>""")
    jump = "".join(
...
def build_workbook():
    ...
    for week in course.WEEKS:
        challenges = "".join(
            f'<div class="card"><strong>Challenge {number}. {esc(challenge["task"])}</strong>'
            f'<p style="margin:8px 0 0">{esc(challenge["detail"])}</p>'
            f'<p class="muted" style="margin:8px 0 0;font-size:13px"><strong>Done when:</strong> {esc(challenge["done"])}</p></div>'
            for number, challenge in enumerate(week["homework"], start=1)
        )
        recap = "".join(f"<li>{esc(recap_item)}</li>" for recap_item in week["recap"])
...
<h3>Words to know</h3>
<p>{"".join(f'<span class="pill">{esc(concept)}</span>' for concept in week["new_concepts"])}</p>
<h3>Challenges</h3>
{challenges}
<h3>Stuck?</h3>
...
def playable_state(desc):
    """The panel code one checkpoint descriptor will run."""
    return state_at(desc["week"]) if desc["done"] is None else state_upto(desc["week"], desc["done"])


def playable_page(week_n, done=None):
    """The game as it stands at one point in week_n, as a standalone playable
    page, with the console bridge on top so the deck can show what it logs and
    where it fails. `done` is the set of (panel, block) the class has typed by
    this slide; None means the whole week.

    This is the biggest difference from AI101. PixelPad runs entirely in the
    browser off a CDN - no server, no account, no key - so a checkpoint slide
    does not show a picture of what the class should have. It PLAYS it.
    """
    panels = playable_state({"week": week_n, "done": done})
    return pixelpad.playable_html(panels, course.SPRITES, course.ROOMS,
                                  "Week %d" % week_n, guard_html=CP_BRIDGE)
...
def balance_quiz(quiz):
    ...
    target = int(hashlib.md5(quiz["q"].encode("utf-8")).hexdigest(), 16) % option_count
    others = [option for index, option in enumerate(opts) if index != quiz["answer"]]
    new = []
    oi = 0
...
def slide_plan(week, seen):
    ...
    out = []
    # Which (panel, block) pairs the class has finished typing so far this week.
    # It is what makes a mid-hour checkpoint run the game they actually have.
    done, checkpoints = set(), 0
    for spec in week["slides"]:
        if spec.get("checkpoint"):
            # A "run it and check" slide: the deck renders the game as it stands
            # at THIS point, so the class can compare their screen to it. An
            # explicit checkpoint_week means "the whole of that week" instead.
            checkpoints += 1
            out.append(("checkpoint", {"week": spec.get("checkpoint_week", week["n"]),
                                       "done": None if spec.get("checkpoint_week")
                                               else frozenset(done),
                                       # unique per slide: two checkpoints in one
                                       # week used to share a DOM id, so the
                                       # second one's game rendered into the first
                                       "cid": "cp%d-%d" % (week["n"], checkpoints),
        ...
        if not expanded:
            ...
            continue
        eyebrow_for = {"py": "Python", "game": "PixelPad", "art": "Drawing"}
        ...
        for filename, block in refs:
            ...
            for index, line in enumerate(lines):
                lineno = start + index
                emit_deletes(lineno, lines[:index])   # a line removed from this spot
                if not line.strip():
                    continue
                ...
                note = notes[index] if notes and index < len(notes) else ""
                if note is None:
            ...
            if shown > 1:
                ...
                while whole and not whole[-1].strip():
                    whole.pop()
                out.append(("block", {"file": filename, "start": start, "lines": whole,
                                      "marks": marks, "block": block,
                                      "done": frozenset(done)}))
...
def _vbox(left, label, cls=""):
    """A list/array cell: rounded box with centred monospace text."""
    return ('<g class="vb {cls}"><rect class="vb-rect" x="{left}" y="{top}" '
            'width="{width}" height="{height}" rx="13"/>'
            '<text class="vb-txt" x="{text_x}" y="{text_y}">{caption}</text></g>').format(
                cls=cls, left=left, top=_ROWY, width=_BW, height=_BH,
                text_x=left + _BW // 2, text_y=_ROWY + _BH // 2, caption=esc(label))


def _vtile(left, top, width, height, label, cls=""):
    """A small labelled tile (function input/output, network node, string)."""
    return ('<g class="vtile {cls}"><rect x="{left}" y="{top}" width="{width}" '
            'height="{height}" rx="10"/>'
            '<text x="{text_x}" y="{text_y}">{caption}</text></g>').format(
                cls=cls, left=left, top=top, width=width, height=height,
                text_x=left + width // 2, text_y=top + height // 2, caption=esc(label))
...
def concept_visual(vis):
    ...
    if kind in ("arr-add", "arr-remove"):
        ...
        if kind == "arr-add":
            ...
            startx = (_VW - totalw) // 2
            if end == "right":
                xs = [startx + index * _PITCH for index in range(count)]
                fin = startx + count * _PITCH
                off = _VW + 140 - fin
            else:
                xs = [startx + (index + 1) * _PITCH for index in range(count)]
                fin = startx
                off = -(fin + _BW + 120)
            for index, it in enumerate(items):
                body.append(_vbox(xs[index], it))
            ...
            totalw = count * _BW + (count - 1) * _GAP
            startx = (_VW - totalw) // 2
            xs = [startx + index * _PITCH for index in range(count)]
            li = count - 1 if end == "right" else 0
            off = 240 if end == "right" else -240
            for index, it in enumerate(items):
                if index == li:
                    body.append('<g class="%s-out">%s</g>' % (cid, _vbox(xs[index], it, "vb-in")))
                elif end == "left":
                    body.append('<g class="%s-rest">%s</g>' % (cid, _vbox(xs[index], it)))
                else:
                    body.append(_vbox(xs[index], it))
        ...
        for index, it in enumerate(items):
            body.append(_vbox(startx + index * _PITCH, it))
        seg = 100 // count
        frames = []
        for index in range(count):
            pct, hold = index * seg, index * seg + max(1, seg // 2)
            frames.append("%d%%{transform:translateX(%dpx)}" % (pct, index * _PITCH))
            frames.append("%d%%{transform:translateX(%dpx)}" % (hold, index * _PITCH))
        frames.append("100%{transform:translateX(0)}")
        kf(cid + "k", "".join(frames))
        anim(cid + "-hi", cid + "k", "%.1f" % max(2.6, count * 0.9))
        ...
        kf(cid + "i", "0%%{transform:translateX(0);opacity:0}9%%{opacity:1}"
           ...
           "100%%{opacity:0}" % (travel, travel + 20))
        ...
        kf(cid + "o", "0%%{transform:translateX(-%dpx);opacity:0}52%%{transform:translateX(-%dpx);opacity:0}"
           ...
           "100%%{transform:translateX(-%dpx);opacity:0}" % (travel, travel, travel))
        ...
        width, height = 380, 156
        box_x, box_y = (_VW - width) // 2, (_VH - height) // 2
        rects = {
            "margin": (box_x, box_y, width, height),
            "border": (box_x + 26, box_y + 26, width - 52, height - 52),
            "padding": (box_x + 50, box_y + 50, width - 100, height - 100),
        }
    ...
    elif kind == "box":                                   # a variable = a labelled box
        bw, bh, by = 220, 90, 106
        bx = (_VW - bw) // 2
    ...
    elif kind == "pick":                                  # a pointer picking one cell
        ...
        tx = startx + at * _PITCH
        for index, it in enumerate(items):
            body.append(_vbox(startx + index * _PITCH, it, "vb-pick" if index == at else ""))
    ...
    elif kind == "fork":                                  # a check that branches two ways
        cxc = _VW // 2
    ...
    elif kind == "event":                                 # a click sparks code to run
    ...
    elif kind == "dom":                                   # a child node nests in a parent
        pw, ph = 320, 128
    ...
    elif kind == "tag":                                   # opening + closing tags hug content
        name = vis.get("tag", "p")
        if vis.get("selfclose"):
            ...
            ow, cw, conw, gap = 130, 130, 230, 12
            total = ow + conw + cw + 2 * gap
            sx = (_VW - total) // 2
            body.append('<g class="%s-o">%s</g>' % (cid, _vtile(sx, _ROWY, ow, 64, openl, "vt-tag")))
            body.append(_vtile(sx + ow + gap, _ROWY, conw, 64, vis.get("content", "text")))
            body.append('<g class="%s-c">%s</g>' % (cid, _vtile(sx + ow + gap + conw + gap, _ROWY, cw, 64, closel, "vt-tag")))
    ...
    elif kind == "swatch":                                # a colour that shifts
        target = vis.get("target", "box")
        colour_from, colour_to = vis.get("a", "#1f6feb"), vis.get("b", "#7c5cff")
        bw, bh = 280, 108
        ...
        if target == "text":
            ...
            kf(cid + "k", "0%%{fill:%s}50%%{fill:%s}100%%{fill:%s}" % (colour_from, colour_to, colour_from))
        elif target == "border":
            body.append('<rect class="%s-sw" x="%d" y="%d" width="%d" height="%d" rx="14" fill="#132833" stroke="%s" stroke-width="7"/>' % (cid, bx, by, bw, bh, colour_from))
            kf(cid + "k", "0%%{stroke:%s}50%%{stroke:%s}100%%{stroke:%s}" % (colour_from, colour_to, colour_from))
        else:
            body.append('<rect class="%s-sw" x="%d" y="%d" width="%d" height="%d" rx="14" stroke="#33586a" stroke-width="2" fill="%s"/>' % (cid, bx, by, bw, bh, colour_from))
            kf(cid + "k", "0%%{fill:%s}50%%{fill:%s}100%%{fill:%s}" % (colour_from, colour_to, colour_from))
        ...
        body.append('<text x="%d" y="%d" style="font:700 20px Consolas,monospace;text-anchor:middle;fill:#8fb3bd">%s</text>'
                     % (_VW // 2, by + bh + 26, esc(vis.get("label", colour_to))))

    elif kind == "resize":                                # a box that grows on one axis
        bw, bh = 128, 128
    ...
    elif kind == "round":                                 # corners rounding off
        bw, bh = 150, 150
    ...
    elif kind == "textsize":                              # text growing
    ...
    elif kind == "linegap":                               # line spacing opening up
    ...
    elif kind == "motion":                                # something moving on its own
        cy = _VH // 2
    ...
    elif kind == "scroll":                                # content scrolling in a viewport
        vw2, vh2 = 300, 140
        ...
        rowsn, lh = 8, 30
        inner = "".join('<rect x="%d" y="%d" width="%d" height="16" rx="4" fill="#2a4a57"/>' % (vx + 18, vy + 14 + index * lh, vw2 - 70) for index in range(rowsn))
    ...
    elif kind == "card":                                  # an object as a key:value card
        ...
        for index, (kk, vv) in enumerate(rows):
            ry = cy + 22 + index * rh
        ...
        frames = []
        for index in range(count):
            pct, hold = index * (100 // count), index * (100 // count) + (100 // count) // 2
            frames.append("%d%%{transform:translateY(%dpx)}" % (pct, index * rh))
            frames.append("%d%%{transform:translateY(%dpx)}" % (hold, index * rh))
        frames.append("100%{transform:translateY(0)}")
        kf(cid + "k", "".join(frames))
        css.append(".%s-hl{opacity:.55;animation:%sk %.1fs ease-in-out infinite}" % (cid, cid, max(2.4, count * 0.9)))

    elif kind == "tiles":                                 # labelled parts of a line, swept
        parts = vis["parts"]
        count = len(parts)
        ws = [max(84, 28 + len(str(part)) * 15) for part in parts]
        gap = 14
        total = sum(ws) + gap * (count - 1)
        left = (_VW - total) // 2
        xs = []
        for tile_width in ws:
            xs.append(left)
            left += tile_width + gap
        for index, part in enumerate(parts):
            body.append(_vtile(xs[index], _ROWY, ws[index], 64, part))
        dur = max(2.4, count * 0.85)
        kf(cid + "k", "0%{opacity:0}12%{opacity:.85}26%{opacity:0}100%{opacity:0}")
        for index in range(count):
            body.append('<rect class="%s-h%d vb-hi" x="%d" y="%d" width="%d" height="76" rx="12"/>' % (cid, index, xs[index], _ROWY - 6, ws[index]))
            css.append(".%s-h%d{opacity:0;animation:%sk %.1fs ease-in-out infinite;animation-delay:%.2fs}" % (cid, index, cid, dur, index * dur / count))

    elif kind == "flex":                                  # items settling into a layout
        ...
        def _spread(gap):
            tot = count * tw + (count - 1) * gap
            sx = (_VW - tot) // 2
            return [(sx + index * (tw + gap), rowy) for index in range(count)]
        ...
        if mode == "gap":
            ...
            start = _spread(20)
            th = 50
            ch = count * th + (count - 1) * 8
            sy = (_VH - ch) // 2
            target = [((_VW - tw) // 2, sy + index * (th + 8)) for index in range(count)]
        elif mode == "end":
            left = [(40 + index * (tw + 16), rowy) for index in range(count)]
            start = left[:]
            target = left[:]
            ...
            def _rowpos(idxs, row_y):
                row_count = len(idxs)
                tot = row_count * tw + (row_count - 1) * 16
                sx = (_VW - tot) // 2
                return {idxs[slot]: (sx + slot * (tw + 16), row_y) for slot in range(row_count)}
            tpos = {}
            tpos.update(_rowpos(list(range(per)), rowy - 40))
            tpos.update(_rowpos(list(range(per, count)), rowy + 40))
            start = [((_VW - tw) // 2, rowy)] * count
            target = [tpos[index] for index in range(count)]
        else:  # row
            start, target = [((_VW - tw) // 2, rowy)] * count, _spread(20)
        for index, it in enumerate(items):
            origin, destination = start[index], target[index]
            body.append('<g class="%s-i%d">%s</g>' % (cid, index, _vtile(origin[0], origin[1], tw, th, it)))
            kf("%si%d" % (cid, index), "0%%{transform:translate(0,0)}42%%{transform:translate(%dpx,%dpx)}"
            ...
            anim("%s-i%d" % (cid, index), "%si%d" % (cid, index), "3.6")
...
# AI101 follows a concept's animation with a wireframe slide ringing where that
# piece lives on the page. PXP101 has none on purpose: its checkpoint is the real
# game at that exact week, running in the slide, which shows placement better
# than any wireframe could.
...
def deck_render_html(desc):
    ...
    kind, detail = desc
    if kind == "concept":
        eyebrow = f'<p class="eyebrow">{esc(detail["eyebrow"])}</p>' if detail.get("eyebrow") else ""
        sub = ""
        if detail.get("sub"):
            code_ish = any(mark in detail["sub"] for mark in ("(", "{", "[", "=", ".", "/", ":", ";", "<"))
            sub = '<p class="sub{0}">{1}</p>'.format(" mono" if code_ish else "", esc(detail["sub"]))
        ...
        if detail.get("vis"):
            ...
            return ('<section class="slide concept-vis">{0}<h2>{1}</h2>{2}{3}{4}</section>').format(
                eyebrow, esc(detail["title"]), sub, concept_visual(vis), cap)
        pts = "".join("<li>{0}</li>".format(esc(pt)) for pt in detail.get("bullets", []))
        pts = '<ul class="pts">{0}</ul>'.format(pts) if pts else ""
        return '<section class="slide">{0}<h2>{1}</h2>{2}{3}</section>'.format(
            eyebrow, esc(detail["title"]), sub, pts)
    if kind == "checkpoint":
        page = playable_page(detail["week"], detail["done"]).replace("</script", "<\\/script")
        cid = detail["cid"]
        return ('<section class="slide checkpoint">'
                '<p class="eyebrow">Checkpoint &middot; run it</p>'
                '<h2>{0}</h2><p class="cp-say">{1}</p>'
                '<button class="cp-run" data-cp="{2}">&#9654; Play it</button>'
                '<div class="cp-out" id="out-{2}"></div>'
                '<script type="text/plain" id="src-{2}">{3}</script>'
                '</section>').format(esc(detail["title"]), esc(detail["say"]), cid, page)
    if kind in ("quiz", "quizanswer"):
        quiz = detail["quiz"]
        answered = kind == "quizanswer"
        opts = "".join(
            '<li class="{0}">{1}</li>'.format(
                "right" if (answered and index == quiz["answer"]) else "", esc(opt))
            for index, opt in enumerate(quiz["options"]))
        tail = ('<p class="quiz-why">{0}</p>'.format(esc(quiz["why"]))
                if answered else
        ...
        return ('<section class="slide quiz{0}"><p class="eyebrow">{1}</p>'
                '<h2>{2}</h2><ol class="opts">{3}</ol>{4}</section>').format(
                    ...
                    esc(quiz["q"]), opts, tail)
    if kind == "chunk":
        return deck_code_slide(detail["file"], detail["start"], detail["lines"], detail["marks"], detail["gone"],
    ...
    if kind == "block":
        return ('<section class="slide dark line whole">'
                ...
                'same before moving on.</p></section>').format(
                    esc(detail["file"]), _code_table_html(detail["start"], detail["lines"], None, detail["marks"]))
    if kind == "delete":
        rows = ['<tr class="ctx"><td class="ln">{0}</td><td>{1}</td></tr>'.format(
                    detail["start"] + offset, esc(line) or "&nbsp;") for offset, line in enumerate(detail["lines"])]
        ...
        for offset, line in enumerate(detail["dropped"]):
            rows.append('<tr class="gone hi"><td class="ln">{0}</td><td>{1}</td></tr>'.format(
                detail["at"] + offset, esc(line) or "&nbsp;"))
        count = len(detail["dropped"])
        what = "a line" if count == 1 else "%d lines" % count
        label = ("replace " if detail.get("replacing") else "delete ") + what
        return ('<section class="slide dark line del">'
                ...
                '<table class="code">{2}</table><p class="linenote">{3}</p></section>').format(
                    esc(detail["file"]), label, "".join(rows), esc(detail["note"]))
    ...
    return ('<section class="slide dark line">'
            ...
            '{2}<p class="linenote">{3}</p></section>').format(
                esc(detail["file"]), detail["start"] + detail["hi"],
                _code_table_html(detail["start"], detail["lines"], detail["hi"], detail["marks"]),
                esc(detail["note"]))
...
def build_slides():
    ...
    def code_slide(deck, filename, start, lines, marks, gone, part, parts):
        ...
        total = len(lines) + sum(len(removed) for removed in gone.values())
    ...
    def ctx_slide(deck, filename, start, lines, hi, note):
        ...
        for index, line in enumerate(lines):
            ...
            gut = para.add_run(); gut.text = f"{start + index:>4}  "
    ...
    for week in course.WEEKS:
        ...
        for kind, detail in slide_plan(week, seen):
            if kind == "concept":
                concept_slide(deck, {"title": detail["title"], "sub": detail["sub"], "bullets": detail["bullets"]}, eyebrow=detail.get("eyebrow", ""))
            elif kind == "checkpoint":
                # PowerPoint cannot run a game, so it says where the real one is.
                concept_slide(deck, {"title": detail["title"], "sub": detail["say"],
                                     "bullets": ["Press Play in PixelPad.",
                                                 "The web deck has a Play it button that runs this exact week.",
                                                 "Everyone's game should do the same thing."]},
                              eyebrow="Checkpoint - press Play")
            elif kind == "chunk":
                code_slide(deck, detail["file"], detail["start"], detail["lines"], detail["marks"], detail["gone"], detail["part"], detail["parts"])
            elif kind == "block":
                ctx_slide(deck, detail["file"], detail["start"], detail["lines"], None,
                          "That is the whole piece. Check yours looks the same before moving on.")
            elif kind == "delete":
                del_slide(deck, detail["file"], detail["start"], detail["lines"], detail["dropped"], detail["note"],
                          detail.get("replacing", False))
            elif kind in ("quiz", "quizanswer"):
                quiz = detail["quiz"]
                answered = kind == "quizanswer"
                letters = "ABCDEFGH"
                bullets = [f"{letters[index]}. {opt}" + (" (correct)" if answered and index == quiz["answer"] else "")
                           for index, opt in enumerate(quiz["options"])]
                bullets.append(quiz["why"] if answered else "Pick one - the answer is on the next slide.")
                concept_slide(deck, {"title": quiz["q"], "sub": "", "bullets": bullets},
                              eyebrow="Answer" if answered else "Quick check")
            else:  # linectx
                ctx_slide(deck, detail["file"], detail["start"], detail["lines"], detail["hi"], detail["note"])
...
.cp-out iframe{width:100%;height:min(56vh,520px);border:1px solid #cbd9dd;border-radius:8px;background:#0f1320}
...
// Checkpoint slides: play the real game at that week's state, so the class can
// compare theirs against it. It boots from the PixelPad CDN and needs nothing else.
[].slice.call(document.querySelectorAll('.cp-run')).forEach(function (btn) {
  btn.onclick = function (e) {
    ...
    if (out.firstChild) {
      ...
      out.innerHTML = ''; btn.textContent = '▶ Play it'; return;
    }
    ...
    // allow-same-origin so the engine can use canvas and storage; the page is
    // generated from this course's own code, nothing student-supplied.
...
def build_html_decks():
    ...
    for week in course.WEEKS:
        ...
        dots = "".join("<i></i>" for _slide in slides)
...
def build_milestones():
    ...
    This is what lets an instructor catch up a child who missed a week: the
...
def build_slide_states():
    ...
    for week in course.WEEKS:
        ...
        snaps = [{"slide": 0, "files": state_upto(week_number, frozenset())}]   # floor: prior weeks
        for pos, (kind, detail) in enumerate(plan):
            idx = pos + 1               # deck index; slide 0 is the week-title slide
            # slide_plan is the single source of truth for what has been typed
            # by a given slide, so the deck and these snapshots cannot disagree.
            if kind in ("block", "checkpoint") and detail.get("done") is not None:
                snaps.append({"slide": idx, "files": state_upto(week_number, detail["done"])})
...
# the child's book
#
# The workbook is challenges and recap; THIS is the build. One chapter per week:
# what we are making, the sprites to draw at the exact size, then every step as
# a sentence, the lines to type with the new ones green, and a Press Play. It is
# the artifact a child can work from with an adult who has never coded either.
# --------------------------------------------------------------------------
...
def build_textbook():
    """textbook.html - the child's step-by-step book, one chapter per week."""
    seen = set()
    chapters = []
    for week in course.WEEKS:
        wk = week["n"]
        ...
        draw = ""
        if week.get("draw"):
            # The name and the size, and deliberately not the colour. The
            # colour in SPRITES exists so this build has something to run the
            # demo games against - it is a stand-in for a picture nobody has
            # drawn yet. Printing it as a swatch told a child their monster is
            # supposed to be green, which is the opposite of the lesson.
            cards = "".join(
                '<div class="sp"><code>{name}</code>'
                '<p class="muted" style="margin:6px 0 0;font-size:13px">'
                'Draw it {width} wide and {height} tall.</p></div>'.format(
        ...
        for beat in week["flow"]:
            ...
            for index, line in enumerate(lines):
                if not line.strip() or (start + index) not in marks:
                    continue
                for key in course.line_concepts(panel, line):
                    concept = course.CONCEPTS.get(key)
                    if concept and key not in seen:
                        seen.add(key)
                        words.append(
    ...
    body = (
        ...
        '<nav class="jump" aria-label="Jump to a chapter"><span>Chapter</span>{jump}</nav>'
        '<button class="printbtn pill" onclick="window.print()">Print to PDF</button>'
        '<div class="warn"><h3 style="margin-top:0">How to use this book</h3>'
        '<p>Do one chapter a week, in order. Every step tells you which panel to open and exactly '
        'what to type &mdash; the <b>green</b> lines are the new ones. Type them, then press Play. '
...
def main():
    if len(course.WEEKS) != TOTAL_WEEKS:
        raise SystemExit(f"expected {TOTAL_WEEKS} weeks, found {len(course.WEEKS)}")
    ...
    # The bonus track carries its own budget, because "move it to the bonus" is
    # the answer to a full base game and that answer stops working the moment
    # the bonus is unbounded too. Bonus code obeys the same kid rules - a bonus
    # may not teach what the base game forbids - and is counted separately
    # because it never enters state_at().
    bonus_total = 0
    for week in course.WEEKS:
        for panel, lines in bonus_lines(week):
    ...
    if bonus_total > BONUS_BUDGET:
        raise SystemExit(
            ...
            f"A bonus is what a fast finisher does in the last ten minutes, not a second course.")

    # RULES.md, enforced rather than remembered. Each of these confused a real
    # nine-year-old, and this course is for eight-year-olds: a list literal, a
    # division, an `if` sharing its line, a name with no self. or Game. in front.
    # Checked on the FINAL state, so every week's code is covered.
    complaints = []
    for panel in FILES:
        if final[panel]:
    ...
    for week in course.WEEKS:
        ...
        touched = {(filename, block_id)
                   for _kind, filename, block_id, _lines in week["ops"]}
        ...
        if orphans:
            raise SystemExit(
                ...
                + ", ".join(f"{filename}:{block_id}" for filename, block_id in sorted(orphans))
            )
        ...
        if unshown:
            raise SystemExit(
                ...
                + ", ".join(f"{filename}:{block_id}" for filename, block_id in sorted(unshown))
            ...
            for spec in week["slides"]:
                for filename, block in (spec.get("code") or []):
                    ...
                    for offset, line in enumerate(lines):
                        ...
                        if not line.strip() or (start + offset) not in marks:
                            continue
                        ...
                        if not note:
                            raise SystemExit(
                                ...
                                f"line {start + offset} has no LINE_NOTES entry"
                            )

        grew = line_count(state_at(week["n"])) - (line_count(state_at(week["n"] - 1)) if week["n"] > 1 else 0)
        # An eight-year-old types perhaps a line a minute and has to press Play
        # in between. A week over the cap is not a hard week, it is a week that
        # will not finish - split it.
        if grew > WEEK_LINE_CAP:
            raise SystemExit(
                f"week {week['n']} adds {grew} lines, over the {WEEK_LINE_CAP}-line cap - "
                f"split it across two weeks"
            )
        print(f"  week {week['n']:2d}  +{grew:3d} lines  {week['title']}")

    # Two checkpoints in an hour that run identical code are one checkpoint and
    # one wasted minute - and it is how a stray checkpoint_week hid itself: the
    # "push it off the side, it escapes!" slide silently ran the finished week,
    # complete with the clamp taught after it, so nothing escaped.
    seen_plan = set()
    for week in course.WEEKS:
        runs = {}
        for kind, detail in slide_plan(week, seen_plan):
            if kind != "checkpoint":
                continue
            code = tuple(sorted(playable_state(detail).items()))
            if code in runs:
                raise SystemExit(
                    f"week {week['n']}: checkpoints {runs[code]!r} and {detail['title']!r} "
                    f"run identical code - move one, or drop it"
                )
            runs[code] = detail["title"]
    ...
    for key, quiz_set in getattr(course, "QUIZZES", {}).items():
        for quiz in quiz_set:
            ...
            if not quiz.get("q") or len(opts) < 2 or not quiz.get("why"):
    ...
    drift = {filename: (web[filename], expected[filename]) for filename in expected if web[filename] != expected[filename]}
    if drift:
        raise SystemExit("deck drift - week: (html, pptx) " + repr(drift))
    print(f"built: index.html, week-01..{TOTAL_WEEKS:02d}.html, teacher.html, "
          f"textbook.html ({chapters} chapters), workbook.html, "
